...
function filterMarkers() {
  ...
  const checked = Array.from(
    document.querySelectorAll('#toolSidebar input[type="checkbox"]:checked')
  ).map((cb) => cb.dataset.tool);

  // Show/hide markers based on checked tools
...
document
  .querySelectorAll('#toolSidebar input[type="checkbox"]')
  .forEach((cb) => {
...
function createMarker(type, xPct, yPct, meta = {}) {
  ...
  el.addEventListener("click", (ev) => {
    ev.stopPropagation();
    const key =
      el.dataset.type === "market"
        ? normName(el.dataset.market || "")
        : el.dataset.type === "well"
        ? normName(el.dataset.well || "")
  ...
  const memKey = keyFor(type, {
    marketName: el.dataset.market,
    wellName: el.dataset.well,
  });
  placementMemory[memKey] = {
    type,
    x: el.dataset.x,
    y: el.dataset.y,
    marketName: el.dataset.market || null,
    wellName: el.dataset.well || null,
  };
...
function readMarkers() {
  ...
  container.querySelectorAll(".marker").forEach((m) => {
    out.push({
      ...
      w: parseInt(m.style.width),
      h: parseInt(m.style.height),
